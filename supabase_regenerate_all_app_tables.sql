...
-- Structured psychometric rows are committed when each questionnaire phase ends.
CREATE OR REPLACE FUNCTION public.persist_psychometric_answers_v4(
  p_session_id UUID, p_pre_answers JSONB, p_post_answers JSONB
...
CREATE OR REPLACE FUNCTION public.commit_stage_transition_v4(
  p_session_id UUID,p_expected_version BIGINT,p_request_id TEXT,p_payload_hash TEXT,
  p_state JSONB,p_pre_answers JSONB,p_post_answers JSONB
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_response JSONB;
BEGIN
  v_response:=public.commit_stage_transition_v3(p_session_id,p_expected_version,p_request_id,p_payload_hash,p_state);
  PERFORM public.persist_psychometric_answers_v4(p_session_id,p_pre_answers,p_post_answers);
  RETURN v_response;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.commit_quality_transition_v4(
  p_session_id UUID,p_expected_version BIGINT,p_request_id TEXT,p_payload_hash TEXT,
  p_state JSONB,p_events JSONB,p_pre_answers JSONB,p_post_answers JSONB
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_response JSONB;
BEGIN
  v_response:=public.commit_quality_transition_v3(p_session_id,p_expected_version,p_request_id,p_payload_hash,p_state,p_events);
  PERFORM public.persist_psychometric_answers_v4(p_session_id,p_pre_answers,p_post_answers);
  RETURN v_response;
END;
$$;
